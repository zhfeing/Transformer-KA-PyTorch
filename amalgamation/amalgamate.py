...
class Amalgamator:
    def __init__(
        ...
        # set up logger
        self.logger = logging.getLogger("amalgamator_rank_{}".format(dist_utils.get_rank()))
        ...
        if use_decay:
            self.logger.info("With decay strategy")
        # for pytorch amp
        self.scaler: amp.GradScaler = None
        if self.amp:
            self.logger.info("Using AMP training")
    ...
    def train_iter(self, x: detr_utils.NestedTensor, targets: List[Dict[str, Any]]):
        ...
        if self.iter % self.train_cfg["print_interval"] == 0 and dist_utils.is_main_process():
            ...
            self.tb_writer.add_scalars("Train/Weighted_loss_dict", weighted_loss_dict, self.iter)
            self.tb_writer.add_scalars("Train/Error_dict", error_dict, self.iter)
            self.tb_writer.add_scalar("DecayStrategy", self.decay_strategy(self.epoch), self.iter)
        dist_utils.barrier()
        self.iter += 1
    ...
    def validate_and_save(self):
        ...
        if dist_utils.is_main_process():
            ...
            # write tb logger
            self.tb_writer.add_scalar("Loss/Val", loss, self.iter)
            self.tb_writer.add_scalar("Val/Mean AP-50", mean_ap_50, self.iter)
            self.tb_writer.add_scalar("Val/Mean AP-75", mean_ap_75, self.iter)
            self.tb_writer.add_scalar("Val/Mean AP", mean_ap, self.iter)
            error_dict = dict()
            for k in list(loss_dict.keys()):
                if "error" in k:
                    error_dict[k] = loss_dict.pop(k)
            self.tb_writer.add_scalars("Val/Loss_dict", loss_dict, self.iter)
            self.tb_writer.add_scalars("Val/Error_dict", error_dict, self.iter)
            label_info = self.val_loader.dataset.label_info
            for cls_id, ap in enumerate(performance["ap"]):
                # ignore background
                if cls_id == 0:
                    continue
                self.tb_writer.add_scalar(f"Val/AP/{label_info[cls_id]}", ap, self.iter)
